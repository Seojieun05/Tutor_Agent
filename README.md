...
Device (laptop mic · browser) → WebSocket → server.py
  ...
  → VLM recognition + whitelisted tags (problem_type + concepts, one call)
...
- **Whitelisted tags, one call**: the recognition call also classifies —
  ...
  The separate tagging round trip cost a measured ~7s per new problem; what
  actually mattered survives it: Python re-enforces both whitelists after the
  call, so an invented id never reaches the KB, tags are cached per problem so
  they cannot drift mid-lesson, and the standalone
  [ConceptTagger](tutor/knowledge/tagger.py) remains for reproducing a tag
  offline from `problem_text` alone. Solution *strategies* are deliberately
  not tags.
